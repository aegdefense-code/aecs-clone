...
const MachineCard = ({ machine, variant = 'light', onImageClick }) => {
  const [activeIndex, setActiveIndex] = useState(0);

  const isDark = variant === 'dark';

  // Auto-play feature: switch image every 2 seconds
  useEffect(() => {
    if (machine.images.length <= 1) return;

    const interval = setInterval(() => {
      setActiveIndex((prev) => (prev + 1) % machine.images.length);
    }, 10000);
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`flex flex-col rounded-[32px] overflow-hidden border transition-all duration-300 group shadow-sm hover:shadow-xl ${isDark
        ? 'bg-white/5 border-white/10 hover:border-brand-primary'
        : 'bg-white border-gray-100 hover:border-brand-primary'
        }`}
    >
      {/* Main Image Area */}
      <div className={`relative overflow-hidden flex items-center justify-center p-6 sm:p-10 cursor-zoom-in ${isDark ? 'bg-white/5 h-64' : 'bg-gray-50 h-80'
        }`} onClick={() => onImageClick(machine.images, activeIndex)}>
        <AnimatePresence mode="wait">
          <motion.img
            key={activeIndex}
            src={machine.images[activeIndex]}
            alt={machine.title}
        ...
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
          <div className="bg-white/90 p-3 rounded-full shadow-lg text-brand-primary transform scale-75 group-hover:scale-100 transition-transform">
            <Search size={20} />
          </div>
        </div>
      </div>

      {/* Thumbnails Area - Now Below Main Image */}
      {machine.images.length > 1 && (
        <div className={`flex gap-3 px-8 py-4 overflow-x-auto no-scrollbar ${isDark ? 'bg-black/20' : 'bg-gray-50/50'}`}>
          {machine.images.map((img, i) => (
            <button
              ...
              className={`w-14 h-14 rounded-xl border-2 transition-all flex-shrink-0 overflow-hidden ${activeIndex === i ? 'border-brand-primary scale-105 shadow-md' : 'border-transparent opacity-50 hover:opacity-100'
                }`}
            >
      ...
      <div className={`p-8 sm:p-10 flex flex-col flex-grow ${isDark ? 'text-white' : 'text-gray-900'}`}>
        <h3 className="text-xl sm:text-2xl font-black uppercase tracking-tight mb-4 group-hover:text-brand-primary transition-colors">
          {machine.title}
        </h3>
        <p className={`font-medium leading-relaxed text-sm text-justify italic ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
          {machine.description}
        </p>
      </div>
    </motion.div>
